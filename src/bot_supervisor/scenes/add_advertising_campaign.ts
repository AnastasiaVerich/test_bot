...
import {ConfirmCancelButtons, SupervisorSettingKeyboard} from "../../bot-common/keyboards/keyboard";
...
import {addAdvertisingCampaign, getAdvertisingCampaign} from "../../database/queries_kysely/advertising_campaigns";
...
export async function addAdvertisingCampaignScene(
    ...
    try {
        ...
        const link: string = referralService.generateReferralLink(`campaign__${name}`);
        ...
        if (resultConfirm === BUTTONS_KEYBOARD.ConfirmButton) {
            ...
            const isAdd = await conversation.external(() => addAdvertisingCampaign({
                name: name,
                referral_link: link,
            }))
            if (!isAdd) {
            ...
            await ctx.reply(ADD_ADV_CAMPAIGN_SCENE.SUCCESS
                + `<code>${link}</code>`, {
                parse_mode: 'HTML',
                reply_markup: SupervisorSettingKeyboard(),
            });
...
async function enterNameStep(
    ...
    try {
        ...
        while (true) {
            ...
            const theSameNameCampaign = await conversation.external(() => getAdvertisingCampaign({
                name: result
            }))
...
async function stepConfirm(
    ...
    try {
        await ctx.reply(
            ADD_ADV_CAMPAIGN_SCENE.CONFIRMATION.replace(
                "{campaign_name}",
                name,
            ), {
                parse_mode: "HTML",
        ...
        while (true) {
            const response = await conversation.waitForHears(
                ...
                {
                    otherwise: (ctx) => ctx.reply(ADD_ADV_CAMPAIGN_SCENE.CONFIRMATION_OTHERWISE, {
                        parse_mode: "HTML",
                        reply_markup: ConfirmCancelButtons(),
                    }),
                });
