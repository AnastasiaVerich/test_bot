...
export async function registrationSupervisorScene(
  ...
  try {
    ...
    await conversation.external(() =>
      updateSupervisorByTgAccount(userAccount, {
  ...
  try {
    ...
    while (true) {
      const response = await conversation.waitFor("message:contact", {
        otherwise: (ctx) =>
          ctx.reply(REGISTRATION_SUPERVISOR_SCENE.ENTER_PHONE_OTHERWISE, {
            parse_mode: "HTML",
            reply_markup: sendUserPhone(),
          }),
      });
    ...
    await ctx.reply(
      `${REGISTRATION_SUPERVISOR_SCENE.ENTERED_USER_PHONE} ${phoneNumber}`,
      {
