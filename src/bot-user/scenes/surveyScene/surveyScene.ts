...
export async function surveyScene(
    ...
    try {
        ...
        if (is_survey_lock_now) {
            await ctx.reply(
                `${SURVEY_SCENE.USER_LOCK_UNTIL} ${formatTimestamp(lockUntilTimespan??0)}.`,
                {
        ...
        if (!location) {
            await ctx.reply(SURVEY_SCENE.SOME_ERROR, {
                reply_markup: AuthUserKeyboard(),
            });
            return;
        }
...
async function stepLocation(
    ...
    try {
        await ctx.reply(SURVEY_SCENE.ENTER_LOCATION, {
            parse_mode: "HTML",
        ...
        while (true) {
            const response = await conversation.waitFor("message:location", {
                otherwise: (ctx) => ctx.reply(SURVEY_SCENE.ENTER_LOCATION_OTHERWISE, {
                    parse_mode: "HTML",
                    reply_markup: sendLocation(),
                }),
            });
            ...
            if ('forward_date' in response.message) {
                await ctx.reply(SURVEY_SCENE.ENTERED_NOT_USER_LOCATION, {
                    parse_mode: "HTML",
                    reply_markup: sendLocation(),
                });
                continue
        ...
        console.log("stepRegion: Ошибка:", error);
        return null;
    }
}
