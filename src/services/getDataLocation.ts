...
export async function reverseGeocode(lat: number, lon: number, language: string = 'ru'): Promise<GeocodeResponse> {
    ...
    try {
        ...
        throw new Error(`Failed to fetch geocode data: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
