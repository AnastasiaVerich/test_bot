...
export class PsqlConversationAdapter implements VersionedStateStorage<string, ConversationData> {
    ...
    async write(key: string, value: VersionedState<ConversationData>): Promise<void> {
        try {

            // Предполагаем, что version - это кортеж [0, string | number]
            const versionValue = Array.isArray(value.version) && value.version.length === 2 ? value.version[1] : value.version;
            const version = Number(versionValue);
            if (isNaN(version) || !Number.isInteger(version)) {
                logger.error(`Invalid version value for key ${key}: ${JSON.stringify(value.version)}`);
                throw new Error(`Invalid version value: ${JSON.stringify(value.version)}`);
            }
