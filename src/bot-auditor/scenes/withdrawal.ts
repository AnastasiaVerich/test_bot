...
import { WITHDRAWAL_AUDITOR_SCENE } from "../../bot-common/constants/scenes";
import {
  MyContext,
  MyConversation,
...
export async function withdrawalScene(
  ...
  try {
    const auditorId = await conversation.external(() => getUserId(ctx));
    if (!auditorId) return;
    ...
    const pendingPayment = await conversation.external(() =>
      getAllPendingPaymentByAuditorId(auditorId),
    );

    if (pendingPayment.length > 0) {
      return ctx.reply(WITHDRAWAL_AUDITOR_SCENE.HAS_PENDING_PAYMENT);
    }

    // Проверка баланса пользователя
    const balance = await conversation.external(() =>
      getAuditorBalance(auditorId),
    );
    ...
    if (Number(userTonBalance) === 0) {
      return ctx.reply(WITHDRAWAL_AUDITOR_SCENE.INVALID_BALANCE);
    }
    ...
    if (resultConfirm === BUTTONS_KEYBOARD.ConfirmButton) {
      ...
      await updateAuditorByAuditorId(auditorId, {
        add_balance: -amountRub,
      });

      return ctx.reply(WITHDRAWAL_AUDITOR_SCENE.SUCCESS, {
        reply_markup: AuthAuditorKeyboard(),
      });
    } else {
      return ctx.reply(WITHDRAWAL_AUDITOR_SCENE.CANCELLED, {
        reply_markup: AuthAuditorKeyboard(),
      });
    }
  } catch (error) {
    logger.error("Error in withdrawalScene: " + error);
    await ctx.reply(WITHDRAWAL_AUDITOR_SCENE.SOME_ERROR, {
      reply_markup: BalanceMenu(),
    });
    return;
  }
...
async function stepAmount(
  ...
  try {
    await ctx.reply(WITHDRAWAL_AUDITOR_SCENE.ENTER_AMOUNT, {
      parse_mode: "HTML",
    ...
    while (true) {
      const response = await conversation.waitFor("message:text", {
        otherwise: (ctx) =>
          ctx.reply(WITHDRAWAL_AUDITOR_SCENE.ENTERED_AMOUNT_OTHERWISE, {
            parse_mode: "HTML",
            reply_markup: { remove_keyboard: true },
          }),
      });
      ...
      if (isNaN(amountTON) || amountTON < 0.05 || amountTON > userTonBalance) {
        await ctx.reply(
          WITHDRAWAL_AUDITOR_SCENE.INVALID_AMOUNT.replace(
            "{balance}",
    ...
    await ctx.reply(
      `${WITHDRAWAL_AUDITOR_SCENE.ENTERED_AMOUNT} ${result} TON`,
      {
        reply_markup: { remove_keyboard: true },
      },
    );
...
async function stepWallet(
  ...
  try {
    await ctx.reply(WITHDRAWAL_AUDITOR_SCENE.ENTER_WALLET, {
      parse_mode: "HTML",
    ...
    while (true) {
      const response = await conversation.waitFor("message:text", {
        otherwise: (ctx) =>
          ctx.reply(WITHDRAWAL_AUDITOR_SCENE.ENTER_WALLET_OTHERWISE, {
            parse_mode: "HTML",
            reply_markup: EmptyKeyboard(),
          }),
      });
      ...
      if (wallet?.length < 10) {
        await ctx.reply(WITHDRAWAL_AUDITOR_SCENE.ENTERED_INVALID_WALLET, {
          parse_mode: "HTML",
          reply_markup: EmptyKeyboard(),
        });
        continue;
  ...
  try {
    ...
    while (true) {
      const response = await conversation.waitForHears(
        ...
        {
          otherwise: (ctx) =>
            ctx.reply(WITHDRAWAL_AUDITOR_SCENE.CONFIRMATION_OTHERWISE, {
              parse_mode: "HTML",
              reply_markup: ConfirmCancelButtons(),
            }),
        },
