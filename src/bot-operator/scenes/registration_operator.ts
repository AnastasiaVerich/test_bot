...
export async function registrationOperatorScene(
  ...
  try {
    ...
    logger.error("Error in registrationOperatorScene: " + error);
    await ctx.reply(REGISTRATION_OPERATOR_SCENE.SOME_ERROR, {
      reply_markup: RegistrationKeyboard(),
    });
  }
  ...
  try {
    ...
    while (true) {
      const response = await conversation.waitFor("message:contact", {
        otherwise: (ctx) =>
          ctx.reply(REGISTRATION_OPERATOR_SCENE.ENTER_PHONE_OTHERWISE, {
            parse_mode: "HTML",
            reply_markup: sendUserPhone(),
          }),
      });
    ...
    await ctx.reply(
      `${REGISTRATION_OPERATOR_SCENE.ENTERED_USER_PHONE} ${phoneNumber}`,
      {
...
async function link(ctx: MyConversationContext, chatId: number) {
  try {
    ...
    if (member.status !== "administrator") {
      await ctx.reply(
        "У бота нет доступа к вашей группе, напишите в поддержку.!",
      );
      return;
    }
    if (!member.can_invite_users) {
      await ctx.reply(
        "У бота нет разрешения для приглошения новых пользователей. Напишите в поддержку.",
      );
      return;
    }
