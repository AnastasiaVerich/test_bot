...
import { getAllPendingPaymentByOperatorId } from "../../../database/queries_kysely/pending_payments";
import { getOperatorBalance } from "../../../database/queries_kysely/operators";
...
export async function handleBalance(ctx: MyContext): Promise<any | void> {
  try {
    ...
    const balance = await getOperatorBalance(operatorId);
    if (!balance) {
      return ctx.reply(HANDLER_BALANCE.USER_ID_UNDEFINED);
    }
    const balanceTon = Number((balance / curseTon).toFixed(2));
    const pendingPayment = await getAllPendingPaymentByOperatorId(operatorId);
