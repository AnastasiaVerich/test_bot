...
export async function isInSurveyActive(
    ...
    try {
        ...
        if (userId === undefined && operatorId === undefined) {
            throw new Error(`At least one ( ${Object.keys(params).join(', ')} ) must be provided.`);
        }
...
export async function getActiveSurvey(
    ...
    try {
        const {
            messageId, userId, operatorId, surveyActiveId
        } = params
...
export async function getAllActiveSurveysUnreservedByOperatorId(
    ...
    try {
        ...
        throw new Error('Error getAllActiveSurveysUnreservedByOperatorId: ' + error);
    }
}
    ...
    try {
        ...
        if (reservationMinutes !== undefined) {
            if (reservationMinutes === null) {
                ...
                set.reservation_end = sql<string>`CURRENT_TIMESTAMP + INTERVAL '1 minute' * ${reservationMinutes}` as unknown as string;
            }
        }
