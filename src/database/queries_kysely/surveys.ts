...
import {SurveyActiveType, SurveysType} from "../db-types";
...
export async function getLeastCompletedSurveyForRegion(
    ...
    try {
        const result = await trx
            ...
            .where(
                eb => eb('completion_limit', '-', eb.ref('active_and_completed_count')),
                '>',
                0
            )
...
export async function updateSurvey(
    ...
    try {
        ...
        if (
            increment_count === undefined
            && decrement_count === undefined
        ) {
            throw new Error(
        ...
        if (increment_count !== undefined) {
            set.active_and_completed_count = sql<number>`active_and_completed_count + ${increment_count}` as unknown as number;
        }

        if (decrement_count !== undefined) {
            set.active_and_completed_count = sql<number>`active_and_completed_count - ${decrement_count}` as unknown as number;
        }
