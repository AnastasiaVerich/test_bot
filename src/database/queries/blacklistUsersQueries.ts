...
export async function checkExistInBlockUser(accountId: number | null, phoneNumber: string | null):Promise<BlacklistUser> {
    ...
    try {
        ...
        const result: QueryResult<BlacklistUser> = await db.query(query, [accountId, phoneNumber]);
        return result.rows[0]
    } catch (error) {
        console.log('Error checkExistInBlockUser:', error);
        throw new Error('Error checkExistInBlockUser');
    }
}
