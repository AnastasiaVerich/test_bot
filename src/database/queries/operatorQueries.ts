...
export const getOperatorsByRegionAndStatus = async (regionId: number, status: OperatorStatus): Promise<Operator[]> => {
    try {
        const query = `
          SELECT *
            FROM operators o
...
export const findOperatorByTelegramId = async (operator_id: number | null, phone: string | null, tg_account: string | null): Promise<Operator> => {
    ...
    try {
        ...
        const query = `
           ...
           WHERE operator_id = $1 OR phone = $2 OR tg_account = $3
            ;`
        const result: QueryResult<Operator> = await db.query(query, [operator_id, phone, tg_account]);
        return result.rows[0]
    } catch (error) {
        console.error('Error findOperatorByTelegramId:', error);
        throw new Error('Error findOperatorByTelegramId');
    }
};
export const updateOperatorStatus = async (operatorId: number, status: OperatorStatus): Promise<void> => {
